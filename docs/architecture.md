...
- **Status/category fields are Prisma enums** (since the enum migration). `ReviewRequest` keeps its four *orthogonal* status tracks (`deliveryStatus`, `engagementStatus`, `ratingStatus`, `googleAttributionStatus`) — now enum-typed, not free strings. The enum vocabularies live in **one cross-app source**, `packages/types/src/enums.ts` (readonly tuples + derived unions), which mirrors the Prisma enums 1:1; web/api/worker import from `@rater/types` and never re-declare a status union. DTOs validate with `@IsIn(THE_TUPLE)`. **Accepted cost:** adding a new status value is now a migration (`ALTER TYPE ... ADD VALUE`, which Postgres can't run in a txn with other DDL and can't reorder/remove) — keep such additions in their own isolated migration. **Deliberate exception:** `Event.eventType` stays a `String` so the append-only log accepts new event types without a migration.
