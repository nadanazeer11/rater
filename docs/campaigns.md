...
- `requests/page.tsx` is also a server stub; `requests-list.tsx` (client) reads campaigns via `useCampaigns(locationId)` and passes them to `<RequestReviewButton>` / `<RequestReviewsCsvButton>`, which render `<CampaignSelect>` (in `requests/campaign-select.tsx`) — a dropdown that **always appears as long as the location has at least one campaign**, pre-selecting the default; the chosen id goes into the mutation input. With one campaign the dropdown is locked to it; with several it picks the newest active by default.
