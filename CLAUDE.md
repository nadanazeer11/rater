...
- [docs/sentiment.md](docs/sentiment.md) — Review sentiment: each Google review gets a cached AI sentiment (Claude Haiku), refreshed only when stale (>15d) and only over the newest 100 reviews/location (cost bound); a 6-month sentiment-trend chart on the overview. Stub falls back to star-rating bands.
